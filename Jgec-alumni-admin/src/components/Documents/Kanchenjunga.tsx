...
const Kanchenjunga = () => {
	...
	const [editKanchenjungaDocs, setEditKanchenjungaDocs] = useState<any>();
	...
	return (
		<>
					...
					<>
						{data?.response.map((item: any) => (
							<DocumentCard
								...
								item={item}
								onEdit={setEditKanchenjungaDocs}
			...
			{(openModal || !!editKanchenjungaDocs) && (
				<ModalDocumentsKanchenjunga
					open={openModal || !!editKanchenjungaDocs}
					closed={() => {
						setOpenModal(false);
						setEditKanchenjungaDocs(null);
						refetch();
					}}
					data={editKanchenjungaDocs}
				/>
			)}
		</>
	);
